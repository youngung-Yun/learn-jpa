...
개발 프로필에서는 `V1__init.sql`로 테이블을 만든 뒤 `data.sql`로 다음 데이터를 자동 입력한다.
...
2. 기존 `V1__init.sql`을 `src/main/resources/db/migration/V1__init.sql`로 옮겨 최초 마이그레이션으로 사용한다.
...
5. `spring.sql.init.mode=never`로 `V1__init.sql`/`data.sql` 자동 초기화와 Flyway가 중복 실행되지 않게 한다.
...
- [x] 일반 LAZY 조회에서 N+1을 재현하고 SQL 발생 시점을 설명할 수 있다.
- [x] Fetch Join으로 단건 주문 상세를 조회하고 일반 조회와 비교했다.
- [x] 같은 요구사항을 `@EntityGraph`로 구현하고 Fetch Join과 비교했다.
- [x] `@BatchSize`가 생성하는 `IN` 쿼리와 배치 단위 쿼리 수를 확인했다.
- [x] 컬렉션 Fetch Join과 페이징을 함께 사용할 때의 문제를 설명할 수 있다.
- [x] 상황별로 세 조회 전략 중 하나를 선택하는 근거를 작성했다.
- [x] 요청 DTO에 표준 Bean Validation 제약 조건을 적용했다.
- [x] `@Valid`로 중첩 주문 항목을 검증했다.
- [x] Bean Validation, 도메인 규칙, DB 제약 조건의 책임을 구분할 수 있다.
